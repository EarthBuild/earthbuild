...
import (
	...
	"github.com/earthbuild/earthbuild/cmd/earthbuild/helper"
...
func (app *earthbuildApp) Run(ctx context.Context, console conslogging.ConsoleLogger, startTime time.Time, lastSignal *syncutil.Signal) int {
...
func (app *earthbuildApp) unhideFlags(ctx context.Context) error {
	var err error
	if os.Getenv("earthbuild_AUTOCOMPLETE_HIDDEN") != "" && os.Getenv("COMP_POINT") == "" { // TODO delete this check after 2022-03-01
		// only display warning when NOT under complete mode (otherwise we break auto completion)
		app.BaseCLI.Console().Warnf("Warning: earthbuild_AUTOCOMPLETE_HIDDEN has been renamed to earthbuild_SHOW_HIDDEN\n")
	}
	showHidden := false
	showHiddenStr := os.Getenv("earthbuild_SHOW_HIDDEN")
	if showHiddenStr != "" {
...
func (app *earthbuildApp) run(ctx context.Context, args []string, lastSignal *syncutil.Signal) int {
	defer func() {
	...
	if err != nil {
		...
		case strings.Contains(err.Error(), "security.insecure is not allowed"):
			helpMsg := "earthbuild --allow-privileged (earthbuild -P) flag is required\n"
			app.BaseCLI.Logbus().Run().SetGenericFatalError(
				time.Now(),
			...
			app.BaseCLI.Console().HelpPrintf(helpMsg)
			return 9
		case strings.Contains(err.Error(), errutil.earthbuildGitStdErrMagicString):
			helpMsg := "Check your git auth settings.\n" +
				"Did you ssh-add today? Need to configure ~/.earthbuild/config.yml?\n" +
			...
			gitStdErr, shorterErr, ok := errutil.ExtractearthbuildGitStdErr(err.Error())
			if ok {
...
func (app *earthbuildApp) printCrashLogs(ctx context.Context) {
	...
	if err != nil {
		app.BaseCLI.Console().Warnf("failed fetching earthbuild-buildkit logs: %s\n", err.Error())
	} else {
