...
const earthbuildCurrentVersion = "0.7"
...
// Docker2earthbuild converts an existing Dockerfile in the current directory and writes out an Earthfile in the current directory
// and error is returned if an Earthfile already exists.
func Docker2earthbuild(dockerfilePath, earthfilePath, imageTag string) error {
	...
	targets := [][]string{
		{
			fmt.Sprintf("VERSION %s\n", earthbuildCurrentVersion),
			"# This Earthfile was generated using docker2earthbuild",
	...
	for i, stage := range stages {
		...
		for _, cmd := range stage.Commands {
			...
			if strings.HasPrefix(l, "ADD ") {
				return errors.Errorf("earthbuild does not support ADD, please convert to COPY instead")
			}
...
func GenerateEarthfile(buildContextPath string, dockerfilePath string, imageTags []string, buildArgs []string, platforms []string, target string) (string, error) {
	...
	err = t.Execute(buf, &earthfileTemplateArgs{
		Version:      earthbuildCurrentVersion,
