...
Its reason to exist: an OOM-killed `buildkit-runc` child surfaces only as
`Canceled` or `file already closed` in the job log, which is indistinguishable
from a real test failure until you look at `dmesg` and `free`. This looks.
...
Inside this repository, reference it by path instead:
...
- extra logs — `EXTRA_LOGS`, plus `$RUNNER_TEMP/add-swap.log` when present
