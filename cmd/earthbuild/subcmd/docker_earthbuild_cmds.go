...
func (a *Doc2Earth) action(cliCtx *cli.Context) error {
	a.cli.SetCommandName("docker2earthbuild")
	err := docker2earthbuild.Docker2earthbuild(a.cli.Flags().DockerfilePath, a.earthfilePath, a.earthfileFinalImage)
	if err != nil {
		return err
	}
	fmt.Fprintf(os.Stderr, "An Earthfile has been generated; to run it use: earthbuild +build; then run with docker run -ti %s\n", a.earthfileFinalImage)
	return nil
}
