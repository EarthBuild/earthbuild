Here is a rough diagram of an earth target being built from the perspective of the initial build cli command:
...
Gotchas:
 - If the converter encounters a reference to a different earth target (e.g. `FROM +other-target`, or `BUILD child+target`), it will recurse into `Earthfile2LLB`.
