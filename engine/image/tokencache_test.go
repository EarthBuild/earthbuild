...
// `registry:token` six times and `pin:token` five. Each is a TLS handshake and
// a round trip to a token service, and each asked for a credential the build
// was already holding - a bearer token is good for minutes, and the whole build
// takes less than one.
//
