...
func (a *Build) Cmds() []*cli.Command {
	return []*cli.Command{
		...
		},
		{
			...
			UsageText:   "earthbuild [options] docker-build [--dockerfile <dockerfile-path>] [--tag=<image-tag>] [--target=<target-name>] [--platform <platform1[,platform2,...]>] <build-context-dir> [--arg1=arg-value]",
			...
			Flags: append(a.buildFlags(),
				&cli.StringFlag{
					Name:        "dockerfile",
					Aliases:     []string{"f"},
					EnvVars:     []string{"earthbuild_DOCKER_FILE"},
...
func (a *Build) ActionBuildImp(cliCtx *cli.Context, flagArgs, nonFlagArgs []string) error {
	...
	for secretKey := range secretsMap {
		if !ast.IsValidEnvVarName(secretKey) {
			// TODO If the year is 2024 or later, please move this check into processSecrets, and turn it into an error; see https://github.com/earthbuild/earthbuild/issues/2883
			a.cli.Console().Warnf("Deprecation: secret key %q does not follow the recommended naming convention (a letter followed by alphanumeric characters or underscores); this will become an error in a future version of earthbuild.", secretKey)
		}
	}
	...
	socketProvider, err := socketprovider.NewSocketProvider(map[string]socketprovider.SocketAcceptCb{
		"earthbuild_save_file": getTryCatchSaveFileHandler(localArtifactWhiteList),
		"earthbuild_interactive": func(ctx context.Context, conn io.ReadWriteCloser) error {
			if !termutil.IsTTY() {
	...
	builtinArgs := variables.DefaultArgs{
		earthbuildVersion:  a.cli.Version(),
		earthbuildBuildSha: a.cli.GitSHA(),
	}
	buildOpts := builder.BuildOpt{
		...
		// to differentiate between a user forgetting to run earthbuild -P, versus a remotely referencing an earthfile that requires privileged.
...
func (a *Build) initAutoSkip(ctx context.Context, skipDB bk.BuildkitSkipper, target domain.Target, overridingVars *variables.Scope) (func(), bool, error) {
	...
	targetHash, stats, err := inputgraph.HashTarget(ctx, inputgraph.HashOpt{
		...
		Console:        a.cli.Console(),
		CI:             a.cli.Flags().CI,
		BuiltinArgs:    variables.DefaultArgs{earthbuildVersion: a.cli.Version(), earthbuildBuildSha: a.cli.GitSHA()},
		OverridingVars: overridingVars,
	})
	if err != nil {
...
func (a *Build) actionDockerBuild(cliCtx *cli.Context) error {
	...
	content, err := docker2earthbuild.GenerateEarthfile(buildContextPath, a.cli.Flags().DockerfilePath, a.dockerTags.Value(), buildArgs.Sorted(), platforms, a.dockerTarget)
	if err != nil {
