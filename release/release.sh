...
# earthbuild does not (yet) have fine control over controlling the order of RUN --push commands (they all happen at the end)
...
#     RELEASE_TAG
#   Optional
#     GITHUB_USER         override the default earthbuild github user
#     DOCKERHUB_USER      override the default earthbuild dockerhub user
#     DOCKERHUB_IMG       override the default earthbuild dockerhub image name
#     EARTHBUILD_REPO        override the default earthbuild repo name
#     BREW_REPO           override the default homebrew-earthbuild repo name
#     GITHUB_SECRET_PATH  override the default +secrets/earthbuild-technologies/github/littleredcorvette/token secrets location where the github token is stored
#     PRERELEASE          override the default false value (must be false or true)
#
# examples
#
#  performing a test release to a non-earthbuild location
#    env -i HOME="$HOME" PATH="$PATH" SSH_AUTH_SOCK="$SSH_AUTH_SOCK" RELEASE_TAG=v0.5.10 GITHUB_USER=alexcb DOCKERHUB_USER=alexcb132 EARTHBUILD_REPO=earthbuild BREW_REPO=homebrew-earthbuild-1 ./release.sh
#
...
if [ -z "$earthbuild" ]; then
  ../earthbuild upgrade
  earthbuild="../earthbuild"
fi

test -f "$earthbuild" || (echo "ERROR: earthbuild is set to $earthbuild which does not exist" && exit 1)

# TODO once v 0.7 is fully released, we can remove this
if ! "$earthbuild" secrets --help 2>&1 | grep migrate > /dev/null; then
    echo "you are using an older version of earthbuild, please upgrade to v0.7.X (or build it from main)"
    exit 1
fi
...
export EARTHBUILD_REPO=${EARTHBUILD_REPO:-earthbuild}
...
export EARTHBUILD_STAGING=${EARTHBUILD_STAGING:-false}
...
if [ "$GITHUB_USER" = "earthbuild" ] && [ "$EARTHBUILD_REPO" = "earthbuild" ]; then
    PRODUCTION_RELEASE="true"
    if [ "$DOCKERHUB_HOST" != "docker.io" ]; then
        echo "expected DOCKERHUB_HOST=docker.io but got $DOCKERHUB_HOST"
        exit 1
    fi
    if [ "$DOCKERHUB_USER" != "earthbuild" ]; then
        echo "expected DOCKERHUB_USER=earthbuild but got $DOCKERHUB_USER"
        exit 1
    fi
    if [ "$DOCKERHUB_IMG" != "earthbuild" ]; then
        echo "expected DOCKERHUB_IMG=earthbuild but got $DOCKERHUB_IMG"
        exit 1
    fi
...
"$earthbuild" --build-arg RELEASE_TAG --build-arg SKIP_CHANGELOG_DATE_TEST +release-notes

if [ -n "$GITHUB_SECRET_PATH" ]; then
    GITHUB_SECRET_PATH_BUILD_ARG="--build-arg GITHUB_SECRET_PATH=$GITHUB_SECRET_PATH"
else
    ("$earthbuild" secrets --org earthbuild-technologies --project core ls >/dev/null) || (echo "ERROR: current user does not have access to the earthbuild-technologies core project"; exit 1);
fi
...
if [ "$EARTHBUILD_STAGING" = "true" ]; then
...
earthbuildnext="$(cat ../earthbuild-next)"
if [[ ! "$earthbuildnext" =~ ^[a-zA-Z0-9]{40}$ ]]; then
    echo "../earthbuild-next does not contain a valid git commit hash; got $earthbuildnext"
    exit 1
fi
echo "earthbuildnext is $earthbuildnext"

"$earthbuild" --push --build-arg DOCKERHUB_USER --build-arg DOCKERHUB_IMG --build-arg DOCKERHUB_BUILDKIT_IMG +release-dockerhub --PUSH_PRERELEASE_TAG="$PRERELEASE" --PUSH_LATEST_TAG="$PUSH_LATEST_TAG" --RELEASE_TAG="$RELEASE_TAG"
"$earthbuild" --push --build-arg DOCKERHUB_USER --build-arg DOCKERHUB_IMG --build-arg DOCKERHUB_BUILDKIT_IMG +release-dockerhub --PUSH_PRERELEASE_TAG="false" --PUSH_LATEST_TAG="false" --RELEASE_TAG="$RELEASE_TAG-ticktock" --BUILDKIT_PROJECT=github.com/earthbuild/buildkit:$earthbuildnext
"$earthbuild" --push --build-arg GITHUB_USER --build-arg EARTHBUILD_REPO --build-arg BREW_REPO --build-arg DOCKERHUB_USER --build-arg DOCKERHUB_BUILDKIT_IMG --build-arg RELEASE_TAG --build-arg SKIP_CHANGELOG_DATE_TEST $GITHUB_SECRET_PATH_BUILD_ARG +release-github --PRERELEASE="$GITHUB_PRERELEASE"
...
if [ "$EARTHBUILD_STAGING" = "true" ]; then
    echo "exiting due to EARTHBUILD_STAGING=$EARTHBUILD_STAGING"
    exit 0
fi

echo "homebrew release with gu=$GITHUB_USER; er=$EARTHBUILD_REPO; br=$BREW_REPO; du=$DOCKERHUB_USER; rt=$RELEASE_TAG"
"$earthbuild" --push --build-arg GITHUB_USER --build-arg EARTHBUILD_REPO --build-arg BREW_REPO --build-arg DOCKERHUB_USER --build-arg RELEASE_TAG $GITHUB_SECRET_PATH_BUILD_ARG +release-homebrew

if [ "$PRODUCTION_RELEASE" = "true" ]; then
    echo "releasing to apt under s3://$S3_BUCKET/deb"
    "$earthbuild" --push --build-arg RELEASE_TAG --build-arg S3_BUCKET ./apt-repo+build-and-release

    echo "releasing to yum under s3://$S3_BUCKET/yum"
    "$earthbuild" --push --build-arg RELEASE_TAG --build-arg S3_BUCKET ./yum-repo+build-and-release
else
