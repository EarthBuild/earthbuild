...
import (
	...
	"github.com/earthbuild/earthbuild/cmd/earthbuild/app"
	"github.com/earthbuild/earthbuild/cmd/earthbuild/base"
	"github.com/earthbuild/earthbuild/cmd/earthbuild/common"
	eFlag "github.com/earthbuild/earthbuild/cmd/earthbuild/flag"
...
var (
	...
	GitSha string
	// BuiltBy contains information on which build-system was used (e.g. official earthbuild binaries, homebrew, etc)
	BuiltBy string

	// DefaultBuildkitdImage is the default buildkitd image to use.
	DefaultBuildkitdImage string

	// DefaultInstallationName is the name included in the various earthbuild global resources on the system,
	// such as the ~/.earthbuild dir name, the buildkitd container name, the docker volume name, etc.
	// This should be set to "earthbuild" for official releases.
...
func main() {
	...
	// Load .env into current global env's. This is mainly for applying earthbuild settings.
	// Separate call is made for build args and secrets.
	envFile := eFlag.DefaultEnvFile
	envFileOverride := false
	if envFileFromEnv, ok := os.LookupEnv("EARTHBUILD_ENV_FILE"); ok {
	...
	customPadding, ok := os.LookupEnv("earthbuild_TARGET_PADDING")
	if ok {
	...
	if envutil.IsTrue("earthbuild_FULL_TARGET") {
	...
	earthbuild := app.NewearthbuildApp(cli, rootApp, buildApp, ctx)
	exitCode := earthbuild.Run(ctx, logging, startTime, lastSignal)
	os.Exit(exitCode)
}
