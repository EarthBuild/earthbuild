...
EarthBuild also has some special command-line switches to ensure best practices are followed within your CI. These come *highly* recommended. Enable these with the [`--ci`](../earthly-command/earthly-command.md#--ci) option, which is shorthand for [`--strict`](../earthly-command/earthly-command.md#strict) [`--no-output`](../earthly-command/earthly-command.md#no-output). Note that `--ci` does **not** enable inline caching; to cache build layers in CI you must pass [`--use-inline-cache`](../earthly-command/earthly-command.md#use-inline-cache) and/or [`--save-inline-cache`](../earthly-command/earthly-command.md#save-inline-cache) explicitly (and `--save-inline-cache` only has an effect when images are actually pushed).
