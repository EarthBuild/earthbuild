...
"$frontend" rmi -f earthbuild-test-saveimage:test
"$earthbuild" ./new-behaviour+myimage
...
"$earthbuild" -V ./new-behaviour-remote+myimage-fromtest
"$frontend" inspect myimage:fromtest >/dev/null
if "$frontend" inspect earthbuild-test-saveimage:test >/dev/null; then
    echo "ERROR ($LINENO): earthbuild-test-saveimage:test should not have been saved."   # THIS IS LEGIT BROKEN
    exit 1
fi
...
"$earthbuild" ./new-behaviour-remote+myimage-copytest
"$frontend" inspect myimage:copytest >/dev/null
if "$frontend" inspect earthbuild-test-saveimage:test >/dev/null; then
	echo "ERROR ($LINENO) : earthbuild-test-saveimage:test should not have been save3."
    exit 1
fi
...
"$earthbuild" ./new-behaviour-remote+myimage-buildtest
"$frontend" inspect myimage:buildtest >/dev/null
"$frontend" inspect earthbuild-test-saveimage:test >/dev/null
