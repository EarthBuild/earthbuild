...
To execute the build, we can run `earthbuild +build`:

```
~/workspace/earthbuild/examples/go ❯ earthbuild +build
...
+build | Target github.com/earthbuild/earthbuild/examples/go:docs-vlad-examples+build built successfully
=========================== SUCCESS ===========================
+build | Artifact github.com/earthbuild/earthbuild/examples/go:docs-vlad-examples+build/go-example as local build/go-example
```

And then, we can execute the hello world program:

```
~/workspace/earthbuild/examples/go ❯ ./build/go-example
hello world
```
...
We can then run `earthbuild +docker` to build this target:

```
~/workspace/earthbuild/examples/go ❯ earthbuild +docker     
buildkitd | Found buildkit daemon as docker container (earthbuild-buildkitd)
context | --> local context .
+base | --> FROM golang:1.15-alpine3.13
+base | resolve docker.io/library/golang:1.15-alpine3.13@sha256:7d45a6fc9cde63c3bf41651736996fe94a8347e726fe581926fd8c26e244e3b2 0%
...
+docker | Target github.com/earthbuild/earthbuild/examples/go:docs-vlad-examples+docker built successfully
=========================== SUCCESS ===========================
...
+docker | Image github.com/earthbuild/earthbuild/examples/go:docs-vlad-examples+docker as go-example:latest
+build | Artifact github.com/earthbuild/earthbuild/examples/go:docs-vlad-examples+build/go-example as local build/go-example
```
...
```
~/workspace/earthbuild/examples/go ❯ docker run --rm go-example:latest
hello world
```

Not only you can run your program with earthbuild, but also your unit and integration tests. 

To execute the unit-tests, we can run `earthbuild -P +unit-test`:
...
golang:1.15-alpine3.13 | --> Load metadata linux/amd64
          ...
          +unit-test | --> RUN CGO_ENABLED=0 go test github.com/earthbuild/earthbuild/examples/go
...
To execute the integration-tests, we can run `earthbuild -P +integration-test`:
...
golang:1.15-alpine3.13 | --> Load metadata linux/amd64
   ...
   +integration-test | --> WITH DOCKER (docker-compose config)
    ...
    redis:6.0-alpine | [██████████] resolve docker.io/library/redis:6.0-alpine@sha256:61f3e955fbef87ea07d7409a48a48b069579e32f37d2f310526017d68e9983b7 ... 100%
               +deps | --> SAVE ARTIFACT go.sum +deps/go.sum AS LOCAL go.sum
               +deps | --> SAVE ARTIFACT go.mod +deps/go.mod AS LOCAL go.mod
             context | transferred 1 file(s) for context /var/folders/5f/jkczhmh52g71v8_q34kt2wm80000gn/T/earthbuild-docker-load330575103 (10 MB, 1 file/dir stats)
   ...
   +integration-test | Creating local-redis ... done
             ongoing | ok  egratgithub.com/earthbuild/earthbuild/examples/go  0.006s
...
Finally, to run the build, unit test, integration test and docker image just run `earthbuild -P +all`:
...
golang:1.15-alpine3.13 | --> Load metadata linux/amd64
   ...
   +integration-test | --> WITH DOCKER RUN --privileged CGO_ENABLED=0 go test github.com/earthbuild/earthbuild/examples/go
          ...
          +unit-test | --> RUN CGO_ENABLED=0 go test github.com/earthbuild/earthbuild/examples/go
              ...
              output | [██████████] copying files ... 100%
              output | [██████████] transferring docker.io/earthbuild/examples:go ... 100%
================================ SUCCESS [main] ================================
              +build | Artifact github.com/earthbuild/earthbuild/examples/go:go-integration-test-example+build/go-example as local build/go-example
               +deps | Artifact github.com/earthbuild/earthbuild/examples/go:go-integration-test-example+deps/go.mod as local go.mod
               +deps | Artifact github.com/earthbuild/earthbuild/examples/go:go-integration-test-example+deps/go.sum as local go.sum
             +docker | Image github.com/earthbuild/earthbuild/examples/go:go-integration-test-example+docker as earthbuild/examples:go
             +docker | Did not push earthbuild/examples:go. Use earthbuild --push to enable pushing
```
