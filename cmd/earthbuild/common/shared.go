...
func IsearthbuildBinary(path string) bool {
	// apply heuristics to see if binary is a version of earthbuild
	data, err := os.ReadFile(path)
	if err != nil {
		return false
	}
	if !bytes.Contains(data, []byte("docs.earthbuild.dev")) {
		return false
	}
	if !bytes.Contains(data, []byte("api.earthbuild.dev")) {
		return false
	}
