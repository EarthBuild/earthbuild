...
```bash
earthbuild --artifact github.com/earthbuild/earthbuild/examples/tutorial/go:main+part6/part6 ./part6
```
...
Whenever you need to use `WITH DOCKER` we recommend (though it is not required) that you use earthbuild's own Docker in Docker (dind) image: `earthbuild/dind:alpine-3.19-docker-25.0.5-r0`.
...
var howCoolIsearthbuild = "IceCool"
...
func TestIntegration(t *testing.T) {
	...
	err := rdb.Set(ctx, "howCoolIsearthbuild", howCoolIsearthbuild, 0).Err()
	if err != nil {
		panic(err)
	}

	resultFromDB, err := rdb.Get(ctx, "howCoolIsearthbuild").Result()
	if err != nil {
		panic(err)
	}
	require.Equal(t, howCoolIsearthbuild, resultFromDB)
}
```
...
```bash
earthbuild --allow-privileged +integration-tests
```
...
```bash
earthbuild --artifact github.com/earthbuild/earthbuild/examples/tutorial/js:main+part6/part6 ./part6
```
...
```
Now you can run `earthbuild -P +app-with-api` to run the app and api side-by-side.
</details>
...
mkdir tutorial
cd tutorial
earthbuild --artifact github.com/earthbuild/earthbuild/examples/tutorial/java:main+part6/part6 ./part6
```
...
```bash
earthbuild --artifact github.com/earthbuild/earthbuild/examples/tutorial/python:main+part6/part6 ./part6
```
