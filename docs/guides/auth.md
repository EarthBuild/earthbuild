...
If no `user` or `password` are found, earth will check for entries under [`~/.netrc`](https://everything.curl.dev/usingcurl/netrc.html).
...
You can run earth with `--verbose`, which will provide debugging messages to help understand how a remote earth reference is transformed into a git URL for cloning.

You can additionally enable low-level git debugging in BuildKit, by adding the following to your `~/.earth/config.yml`:
...
The BuildKit logs can be displayed with `docker logs earth-buildkitd`.
...
before issuing earth commands, if you have not already done so in the past. If you run into troubles, [you can find out more about `docker login` here](https://docs.docker.com/engine/reference/commandline/login/).
