...
Older `library/registry` images did not support the DNS-01 challenge; the upstream issue tracking this was [closed in 2024](https://github.com/distribution/distribution/issues/3041). If you need ACME with DNS-01 for your mirror, use a current `registry` image, or terminate TLS in front of the registry with a reverse proxy that handles certificate issuance for you.
...
To configure earth to use the cache, you must edit `~/.earth/config.yml` to include:
...
The next time earth is run, it will detect the configuration change and will restart the `earth-buildkitd` container to reflect these settings.

You can force these settings to be applied, and verify the mirror appears in the BuildKit config by running:

```bash
earth bootstrap && docker exec earth-buildkitd cat /etc/buildkitd.toml
```
