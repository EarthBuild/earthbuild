...
func (cli *CLI) InitFrontend(cliCtx *cli.Context) error {
	...
	earthbuildDir, err := cliutil.GetOrCreateEarthbuildDir(cli.Flags().InstallationName)
	if err != nil {
		return errors.Wrap(err, "failed to get earthbuild dir")
	}
	cli.Flags().BuildkitdSettings.StartUpLockPath = filepath.Join(earthbuildDir, "buildkitd-startup.lock")
