...
| `EARTH_GIT_TAG` | The first git tag pointing at the commit detected within the build context directory. Empty when the commit carries no tag, and empty when no git directory is detected. | `v1.2.3` |                                        |
| `EARTH_GIT_ORIGIN_URL_SCRUBBED` | `EARTH_GIT_ORIGIN_URL` with any credentials removed. Prefer this one wherever the value is printed, saved into an image, or pushed anywhere: a URL that carried a token is a token in the layer. | `https://github.com/bar/buz.git` |                                        |
| `EARTH_CI_RUNNER` | Whether this build is running on EarthBuild's own CI runner. `false` everywhere else, which is everywhere this engine currently runs. | `false` |                                        |
