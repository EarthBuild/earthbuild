...
`FROM earthbuild/dind:alpine-3.24-docker-29.5.3-r1` inherits from an Earthly-supported docker-in-docker (dind) image. This is recommended, because `WITH DOCKER` requires all the Docker binaries (not just the client) to be present in the build environment.
...
It's recommended to use the `earthbuild/dind:alpine-3.24-docker-29.5.3-r1` image for running docker-in-docker. See the best-practices' section on using [with docker](../guides/best-practices.md#use-earthly-dind) for more details.
