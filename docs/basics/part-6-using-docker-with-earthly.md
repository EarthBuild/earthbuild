...
Whenever you need to use `WITH DOCKER` we recommend (though it is not required) that you use EarthBuild's own Docker in Docker (dind) image: `earthbuild/dind:alpine-3.24-docker-29.5.3-r1`.

Notice `WITH DOCKER` creates a block of code that has an `END` keyword. Everything that happens within this block is going to take place within our `earthbuild/dind:alpine-3.24-docker-29.5.3-r1` container.
