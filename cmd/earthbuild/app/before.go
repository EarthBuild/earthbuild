...
func (app *earthbuildApp) before(cliCtx *cli.Context) error {
...
func (app *earthbuildApp) parseFrontend(cliCtx *cli.Context, cfg *config.Config) error {
...
func (app *earthbuildApp) processDeprecatedCommandOptions(cliCtx *cli.Context, cfg *config.Config) error {
	app.warnIfEarth()
	...
	if app.BaseCLI.Flags().ConversionParallelism != 0 {
		app.BaseCLI.Console().Warnf("Warning: --conversion-parallelism and earthbuild_CONVERSION_PARALLELISM is obsolete, please use 'earthbuild config global.conversion_parallelism <parallelism>' instead")
	}

	// command line overrides the config file
	if app.BaseCLI.Flags().GitUsernameOverride != "" || app.BaseCLI.Flags().GitPasswordOverride != "" {
		app.BaseCLI.Console().Warnf("Warning: the --git-username and --git-password command flags are deprecated and are now configured in the ~/.earthbuild/config.yml file under the git section; see https://docs.earthbuild.dev/earthbuild-config for reference.\n")
...
func (app *earthbuildApp) warnIfEarth() {
	if len(os.Args) == 0 {
		return
	}
	binPath := os.Args[0] // can't use os.Executable() here; because it will give us earthbuild if executed via the earth symlink

	baseName := path.Base(binPath)
	if baseName == "earth" {
		app.BaseCLI.Console().Warnf("Warning: the earth binary has been renamed to earthbuild; the earth command is currently symlinked, but is deprecated and will one day be removed.")

		absPath, err := filepath.Abs(binPath)
		if err != nil {
			return
		}
		earthbuildPath := path.Join(path.Dir(absPath), "earthbuild")
		earthbuildPathExists, _ := fileutil.FileExists(earthbuildPath)
		if earthbuildPathExists {
			app.BaseCLI.Console().Warnf("Once you are ready to switch over to earthbuild, you can `rm %s`", absPath)
		}
	}
}
...
func defaultConfigPath(installName string) string {
	earthbuildDir := cliutil.GetearthbuildDir(installName)
	oldConfig := filepath.Join(earthbuildDir, "config.yaml")
	newConfig := filepath.Join(earthbuildDir, "config.yml")
