...
- Fixed earthbuild failing when using a remote docker host from a machine with an incompatible architecture. [#1895](https://github.com/earthbuild/earthbuild/issues/1895)
...
This version promotes a number of features that have been previously in Experimental and Beta status. To make use of the features in this version you need to declare `VERSION 0.6` at the top of your Earthfile. If a version is not declared, then earthbuild's interpreter will assume `VERSION 0.5`.

If you are not ready to update your scripts to take advantage of `VERSION 0.6`, then you may upgrade earthbuild anyway and your scripts should continue to work as before, provided that they either declare `VERSION 0.5` or they don't declare a version at all.
...
It is recommended to use `VERSION 0.6` instead as individual feature flags don't guarantee proper forwards-backwards compatibility. Note, however, that earthbuild `0.5.*` is not able to run a `VERSION 0.6` Earthfile and will return an error.
...
<!--changelog-parser-ignore-start-->
- What earthbuild outputs locally has changed in a way that is not backwards compatible. For an artifact or an image to be produced locally it needs to be part of a `BUILD` chain (or be part of the target being directly built). Artifacts and images introduced through `FROM` or `COPY` are no longer output locally.
...
- The syntax for passing build args has been changed.
  ...
  ```
  earthbuild --build-arg NAME=john +some-target
  ```

  earthbuild v0.6 (new way)

  ```
  earthbuild +some-target --NAME=john
  ```
...
- `.earthbuildignore` no longer includes any implicit entries like `Earthfile` or `.earthbuildignore`. These will need to be specified explicitly. [#1294](https://github.com/earthbuild/earthbuild/issues/1294)
...
- earthbuild now performs local image outputs to the local Docker daemon through a built-in registry. This speeds up the process drastically as common layers no longer need to be transferred over [#500](https://github.com/earthbuild/earthbuild/issues/500).
- earthbuild now enables additional parallelism to speed up certain operations that were previously serialized [#888](https://github.com/earthbuild/earthbuild/issues/888). Note that this setting was previously controlled by `--conversion-parallelism` flag or the `EARTHBUILD_CONVERSION_PARALLELISM` environment variable while in experimental stage. It has now been moved as part of the earthbuild config and has been promoted to GA.
...
- [`WITH DOCKER`](https://docs.earthbuild.dev/docs/earthfile#with-docker) has been promoted to GA [#576](https://github.com/earthbuild/earthbuild/issues/576).
- [`FROM DOCKERFILE`](https://docs.earthbuild.dev/docs/earthfile#from-dockerfile) has been promoted to GA.
- [`LOCALLY`](https://docs.earthbuild.dev/docs/earthfile#locally) has been promoted to GA [#580](https://github.com/earthbuild/earthbuild/issues/580).
- [`RUN --interactive` and `RUN --interactive-keep`](https://docs.earthbuild.dev/docs/earthfile#run) have been promoted to GA [#693](https://github.com/earthbuild/earthbuild/issues/693).
- [`IF`](https://docs.earthbuild.dev/docs/earthfile#if) and [`FOR`](https://docs.earthbuild.dev/docs/earthfile#for) have been promoted to GA [#779](https://github.com/earthbuild/earthbuild/issues/779).
- Support for Apple Silicon M1 has been promoted to GA [#722](https://github.com/earthbuild/earthbuild/issues/722).
- [Multi-platform builds](https://docs.earthbuild.dev/docs/guides/multi-platform) have been promoted to GA [#536](https://github.com/earthbuild/earthbuild/issues/536).
- Mounting secrets as files have been promoted as GA [#579](https://github.com/earthbuild/earthbuild/issues/579).
- [`VERSION`](https://docs.earthbuild.dev/docs/earthfile#version) has been promoted to GA [#991](https://github.com/earthbuild/earthbuild/issues/991)
- [User-defined commands (UDCs)](https://docs.earthbuild.dev/docs/guides/udc) have been promoted to GA [#581](https://github.com/earthbuild/earthbuild/issues/581).
...
- [Shared cache](https://docs.earthbuild.dev/docs/guides/shared-cache) and `--ci` mode are now GA [#11](https://github.com/earthbuild/earthbuild/issues/11).
- New builtin args `USERPLATFORM`, `USEROS`, `USERARCH`, and `USERVARIANT` which represent the platform, OS, architecture, and processor variant of the system earthbuild is being called from [#1251](https://github.com/earthbuild/earthbuild/pull/1251). Thanks to @akrantz01 for the contribution!
...
- Extended auto-completion to be build-arg aware. Typing `earthbuild +my-target --<tab><tab>` now prints possible build-args specific to `+my-target`. [#1330](https://github.com/earthbuild/earthbuild/pull/1330).
...
- Fixed homebrew installation on macOS 12. [#1370](https://github.com/earthbuild/earthbuild/pull/1370), [homebrew/earthbuild#13](https://github.com/earthbuild/homebrew-earthbuild/pull/13)
- `failed due to failed to autodetect a supported frontend` errors will now include underlying reason for failure
- Cache export was not honoring `earthbuild_MAX_REMOTE_CACHE` setting.
- Buildkit logs were not being sent to `earthbuild-buildkitd` container's output.
- kind required permissions were not available in earthbuild-buildkitd.
...
- docker and fsutils versions were set to match versions defined in earthbuild's buildkit fork.
...
- The syntax for passing build args has been changed.
  ...
  This change is part of the [UDC proposal #581](https://github.com/earthbuild/earthbuild/issues/581). The old way of passing args is deprecated and will be removed in a future version (however, it still works in 0.6).
<!--changelog-parser-ignore-end-->
- Add builtin args `USERPLATFORM`, `USEROS`, `USERARCH`, and `USERVARIANT` which represent the platform, OS, architecture, and processor variant of the system earthbuild is being called from [#1251](https://github.com/earthbuild/earthbuild/pull/1251). Thanks to @akrantz01 for the contribution!
- Support for required ARGs (`ARG --required foo`) [#904](https://github.com/earthbuild/earthbuild/issues/904). Thanks to @camerondurham for the contribution!
...
- Extend auto-completion to be build-arg aware. Typing `earthbuild +my-target --<tab><tab>` now prints possible build-args specific to `+my-target`. [#1330](https://github.com/earthbuild/earthbuild/pull/1330).
- Buildkit was updated to `d429b0b32606b5ea52e6be4a99b69d67b7c722b2`. This includes a number of bug fixes, including eliminating crashes due to `panic failed to get edge`.
...
- New `--output` flag, which forces earthbuild to enable outputs, even when running under `--ci` mode [#1200](https://github.com/earthbuild/earthbuild/issues/1200).
...
- Removed spurious `BuildKit and Local Registry URLs are pointed at different hosts (earthbuild-buildkitd vs. 127.0.0.1)` warning.
...
- introduced `COPY --if-exists` which allows users to ignore errors which would have occurred [if the file did not exist](https://docs.earthbuild.dev/docs/earthfile#if-exists).
- introduced new `ip_tables` config option for controlling which iptables binary is used; fixes #1160
- introduced warning message when saving to paths above the current directory of the current Earthfile; these warnings will eventually become errors unless the `--force` [flag](https://docs.earthbuild.dev/docs/earthfile#force) is passed to `SAVE ARTIFACT`.
...
- deb and yum packages no longer clear the earthbuild cache on upgrades


## v0.5.22 - 2021-08-11

- when running under `--ci` mode, earthbuild now raises an error if a user attempts to use the interactive debugger
- updated underlying BuildKit version
- print all request and responses to BuildKit when running under --debug mode
- support for specifying files to ignore under `.earthbuildignore` in addition to `.earthignore`; an error is raised if both exist
- new ARG `EARTHBUILD_GIT_SHORT_HASH` will contain an 8 char representation of the current git commit hash
- new ARG `EARTHBUILD_GIT_COMMIT_TIMESTAMP` will contain the timestamp of the current git commit
- new ARG `EARTHBUILD_SOURCE_DATE_EPOCH` will contain the same value as `EARTHBUILD_GIT_COMMIT_TIMESTAMP` or 0 when the timestamp is not available
...
- Show hidden dev flags when `EARTHBUILD_AUTOCOMPLETE_HIDDEN="1"` is set (or when running a custom-built version).
- Improved crash logs.
...
- Added option to disable analytics data collection when environment variables `earthbuild_DISABLE_ANALYTICS` or `DO_NOT_TRACK` are set.
- Include version and help flags in autocompletion output.

## v0.5.17 - 2021-06-15

- Begin experimental official support for `earthbuild/earthbuild` and `earthbuild/buildkitd` images; including a new `entrypoint` for `earthbuild/earthbuild` (https://github.com/earthbuild/earthbuild/pull/1050)
...
- fixes handling of `Error getting earthbuild dir` lookup failures which prevents earthbuild from running (https://github.com/earthbuild/earthbuild/issues/1026)
- implements ability to perform local exports via buildkit-hosted local registry in order to speed up exports; the feature is currently disabled by default but can be enabled with `earthbuild config global.local_registry_host 'tcp://127.0.0.1:8371'` (https://github.com/earthbuild/earthbuild/issues/500)

## v0.5.15 - 2021-05-27

- `earthbuild config` is no longer experimental. (https://github.com/earthbuild/earthbuild/pull/979)
- Running a target, will now `bootstrap` automatically, if it looks like `earthbuild bootstrap` has not been run yet. (https://github.com/earthbuild/earthbuild/pull/989)
- `earthbuild bootstrap` ensures the permissions on the `.earthbuild` folder are correct (belonging to the user) ( https://github.com/earthbuild/earthbuild/pull/993)
...
- Added `EARTHBUILD_TARGET_PROJECT_NO_TAG` built-in argument (https://github.com/earthbuild/earthbuild/pull/1011)
...
- Adds a retry for remote BuildKit hosts when using the `earthbuild_BUILDKIT_HOST` configuration option. (#952)
- Re-fetch credentials when they expire (#957)
- Make use of `~/.netrc` credentials when no config is set under `~/.earthbuild/config.yml` (#964)
...
- [**experimental**] Improved parallelization when using commands such as `IF`, `WITH DOCKER`, `FROM DOCKERFILE`, `ARG X=$(...)` and others. To enable this feature, pass `--conversion-parallelism=5` or set `EARTHBUILD_CONVERSION_PARALLELISM=5`. (https://github.com/earthbuild/earthbuild/issues/888)
- Auto-detect MTU (https://github.com/earthbuild/earthbuild/issues/847)
- MTU may set via config `earthbuild config global.cni_mtu 12345` (https://github.com/earthbuild/earthbuild/pull/906)
- Hide `--debug` flag since it is only used for development on earthbuild itself
- Download and start buildkitd as part of the earthbuild bootstrap command
...
- fix zsh autocompletion issue for mac users
  If the autocompletion bug persists for anyone (e.g. seeing an error like `command not found: __earthbuild__`), and the issues persists after upgrading to v0.5.7; it might be necessary to delete the _earthbuild autocompletion file before re-running earthbuild bootstrap (or alternatively manually replace `__earthbuild__` with the full path to the earthbuild binary).
...
- Add help text when issuing `earthbuild config <item> --help`. Improved user experience. (https://github.com/earthbuild/earthbuild/pull/814)
...
- unable to run earthbuild in docker container with mounted host-docker socket (fixes #791)
- `./.tmp-earthbuild-outXXXXXX` temp files are now stored under `./.tmp-earthbuild-out/tmpXXXXXX` and are correctly excluded from the build context


## v0.5.2 - 2021-02-18

- New experimental command for editing the earthbuild config (https://github.com/earthbuild/earthbuild/issues/675)
...
- Fix `earthbuild-linux-arm64` binary - was a Mac binary by mistake (https://github.com/earthbuild/earthbuild/issues/789)
...
- Improved errors when parsing invalid Earthfiles (to enable the new experimental code, set the `earthbuild_ENABLE_AST` variable to `true`)

## v0.5.0 - 2021-02-01

- Switch to BSL license. For [more information about this decision, take a look at our blog post](https://blog.earthbuild.dev/every-open-core-company-should-be-a-source-available-company/).
...
- There are now separate `SUCCESS` lines for each of the two possible phases of an earthbuild run: `main` and `push`.
- [Support of popular cloud registries for the experimental shared cache feature is now properly documented](https://docs.earthbuild.dev/guides/shared-cache#compatibility-with-major-registry-providers)
...
- Bug fixes for Apple Silicon. `earthbuild-darwin-arm64` binary is now available. Please treat this version as highly experimental for now. (https://github.com/earthbuild/earthbuild/issues/722)
...
- fixed: `EARTHBUILD_GIT_PROJECT_NAME` contained the raw git URL when HTTPS-based auth was used (fixes #671)
...
- misc: sending anonymized usage metrics to earthbuild
